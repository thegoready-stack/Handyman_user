...
Future<List<ServiceData>> searchServiceAPI({
  ...
  try {
    var res = ServiceResponse.fromJson(await handleResponse(
      await buildHttpResponse(apiUrl),
    ));
  ...
  try {
    ...
    if (status == BOOKING_TYPE_ALL) {
      ...
      res = BookingListResponse.fromJson(await handleResponse(
          await buildHttpResponse('booking-list?$serviceIds$dateStart$dateEnd$providerIds$handymanIds$status$paymentStatuss$paymentTypes$perPageItem$pageCount', method: HttpMethodType.GET)));
    }
